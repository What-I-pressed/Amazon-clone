...
public class ProductService {
    ...
    private Specification<Product> getSpec(String name, Long categoryId, Double lowerBound,
        ...
        Specification<Product> sellerSpec = sellersIds != null ? sellersIds.stream().map(entry -> ProductSpecification.sellerIs(entry)).reduce((root, query, criteriaBuilder) -> criteriaBuilder.conjunction(), Specification::or) : null;
        Specification<Product> slugSpec = slugs != null ? slugs.stream().map(entry -> ProductSpecification.sellerIs(entry)).reduce((root, query, criteriaBuilder) -> criteriaBuilder.conjunction(), Specification::or) : null;
